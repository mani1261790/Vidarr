...
struct PadBrowserRootView: View {
    ...
    private func commitStartNavigation(_ input: String, tabID: UUID) {
        ...
        showBottomBar(persist: true)
        #if DEBUG
        print("[VidarrPad] commitStartNavigation", "tabID:", tabID.uuidString, "input:", trimmed)
        #endif
        DispatchQueue.main.async {
            model.selectTab(id: tabID)
            if model.selectedTab == nil {
                #if DEBUG
                print("[VidarrPad] selectedTab nil before load, stabilizing model")
                #endif
                model.stabilizeForForeground()
                model.selectTab(id: tabID)
            }
            #if DEBUG
            print("[VidarrPad] selectedTab after select:", model.selectedTab?.id.uuidString ?? "nil", "start:", model.selectedTab?.showsNativeStartPage ?? false, "url:", model.selectedTab?.urlString ?? "")
            #endif
